        ...
        while GT_update_is_found:
            ...
            if GT_update_is_found:
                ...
                if is_first_if_condition_satisfied(G, S_plus, S_plus_bar, k, num_clusters, rho_star):
                    ...
                    # # Update core_i = S_plus
                    # core_sets[index_cluster_to_update] = S_plus[:]
                    #
                    # # Update P_i = P_i - S_plus_bar
                    # clusters[index_cluster_to_update] = diff(clusters[index_cluster_to_update], S_plus_bar)
                    #
                    # # Update P_(num_clusters + 1) = core_(num_clusters + 1) = S_plus_bar
                    # clusters.append(S_plus_bar[:])
                    # core_sets.append(S_plus_bar[:])
                ...
                if not GT_update_is_done and is_second_if_condition_satisfied(G, S_plus, S_plus_bar, core_i, k):
                    ...
                    # Update core_i to either S_plus or S_plus_bar of minimum conductance
                    # if graph.conductance(G, S_plus) > graph.conductance(G, S_plus_bar):
                    #     S_plus_bar, S_plus = S_plus, S_plus_bar
                    # core_sets[index_cluster_to_update] = S_plus[:]
                ...
                if not GT_update_is_done and is_third_if_condition_satisfied(G, S_minus, k, num_clusters, rho_star):
                    ...
                    # # Update P_i = P_i - S_minus
                    # clusters[index_cluster_to_update] = diff(clusters[index_cluster_to_update], S_minus)
                    #
                    # # Update P_(num_clusters + 1) = core_(num_clusters + 1) = S_minus
                    # core_sets.append(S_minus[:])
                    # clusters.append(S_minus[:])
                ...
                # At this point only a refinement of the partition is possible, i.e. the last two "ifs" of GT algorithm
                if not GT_update_is_done:
                    ...
                    # If best_cluster_index = index_cluster_to_update, then the forth "if" is not satisfied
                    best_cluster_index = find_cluster_P_j_that_maximises_weight_from_T_to_P_j(G, P_i_minus_core_i, clusters)

                    # Forth "if" in the algorithm.
                    if best_cluster_index != index_cluster_to_update:
                        move_subset_T_from_P_i_to_P_j(P_i_minus_core_i, clusters, index_cluster_to_update, best_cluster_index)
                        #
                        # # Update P_i = core_i
                        # clusters[index_cluster_to_update] = core_i[:]
                        #
                        # # Merge (P_i - core_i) with argmax_(P_j){w(P_i - core_i -> P_j)}
                        # clusters[best_cluster_index] = clusters[best_cluster_index] + diff(P_i, core_i)
                    ...
                    if not GT_update_is_done:
                        ...
                        # If best_cluster_index = index_cluster_to_update, then the fifth "if" is not satisfied
                        ...
                        if best_cluster_index != index_cluster_to_update:
                            ...
                            # clusters[index_cluster_to_update] = diff(clusters[index_cluster_to_update], S_minus)
                            #
                            # # Merge S_minus with argmax_(P_j){w(S_minus -> P_j)}
                            # clusters[best_cluster_index] = clusters[best_cluster_index] + S_minus
        ...
        for i in range(len(clusters)):
            ...
            for node in critical_nodes_of_T_i:
                ...
                if is_sixth_if_condition_satisfied(G, N_plus, N_plus_bar, k, num_clusters, rho_star):
                    ...
                    # # Update core_i = N_plus
                    # core_sets[i] = N_plus[:]
                    #
                    # # Update P_i = P_i - N_plus_bar
                    # clusters[i] = diff(clusters[i], N_plus_bar)
                    #
                    # # Update P_(num_clusters + 1) = core_(num_clusters + 1) = N_plus_bar
                    # core_sets.append(N_plus_bar[:])
                    # clusters.append(N_plus_bar[:])
                ...
                if not overall_update_is_found and is_seventh_if_condition_satisfied(G, N_plus, core_i, k):
                    ...
                    # # Update core_i to either N_plus or N_plus_bar of smallest conductance
                    # if graph.conductance(G, N_plus) > graph.conductance(G, N_plus_bar):
                    #     N_plus_bar, N_plus = N_plus, N_plus_bar
                    # core_sets[i] = N_plus[:]
                    overall_update_is_found = True
                    break
                ...
                if not overall_update_is_found and vol(G, N_minus) <= vol(G, P_i) / 2:
                    ...
                    if weight(G, N_minus, P_i) < weight(G, N_minus, clusters[best_cluster_index]):
                        ...
                        # # Update P_i = P_i - N_minus
                        # clusters[i] = diff(clusters[i], N_minus)
                        #
                        # # Merge N_minus with argmax_(P_j){w(N_minus -> P_j)}
                        # clusters[best_cluster_index] = clusters[best_cluster_index] + N_minus
